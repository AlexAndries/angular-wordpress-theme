...
function ultra_logo($rettype = ""){
	...
	if(isset($ultraadmin['enable-logo']) && $ultraadmin['enable-logo'] != "1" && $ultraadmin['enable-logo'] == "0" && !$ultraadmin['enable-logo']){
		...
		if($csstype != "custom"){
			global $ultra_color;
			$logo = str_replace("PLUGINURL", THEME_URL . 'includes/ultra-admin', $ultra_color[$csstype]['logo']['url']);
			$logo_folded = str_replace("PLUGINURL", THEME_URL . 'includes/ultra-admin', $ultra_color[$csstype]['logo_folded']['url']);
		}
		if($logo == ""){
...
function ultra_favicon(){
	...
	<?php if($ultraadmin['iphone_icon_retina']['url']): ?>
		...
		<link rel="apple-touch-icon-precomposed" sizes="114x114"
		      href="<?php echo $ultraadmin['iphone_icon_retina']['url']; ?>">
	<?php endif; ?>
	...
	<?php if($ultraadmin['ipad_icon_retina']['url']): ?>
		...
		<link rel="apple-touch-icon-precomposed" sizes="144x144"
		      href="<?php echo $ultraadmin['ipad_icon_retina']['url']; ?>">
	<?php endif; ?>
	<?php
}
