...
class CPT {
	 ...
	 */
	public $textdomain = 'standards-theme';
	...
	function __construct($post_type_names, $options = array()){
		...
		// Register taxonomies.
		$this->add_action('init', array(&$this,
			'register_taxonomies'));
		...
		$this->add_filter('manage_edit-' . $this->post_type_name . '_columns', array(&$this,
			'add_admin_columns'));
		// Populate the taxonomy columns with the posts terms.
		$this->add_action('manage_' . $this->post_type_name . '_posts_custom_column', array(&$this,
			'populate_admin_columns'), 10, 2);
		// Add filter select option to admin edit.
		$this->add_action('restrict_manage_posts', array(&$this,
			'add_taxonomy_filters'));
		// rewrite post update messages
		$this->add_filter('post_updated_messages', array(&$this,
			'updated_messages'));
		$this->add_filter('bulk_post_updated_messages', array(&$this,
			'bulk_updated_messages'), 10, 2);
	}
	...
	function register_post_type(){
		...
		$slug = $this->slug;
		// Default labels.
		$labels = array('name' => sprintf(__('%s', $this->textdomain), $plural),
	 ...
	 */
	function register_taxonomy($taxonomy_names, $options = array(), $hierarchical = true){
	...
	function populate_admin_columns($column, $post_id){
		...
		switch($column){
			...
			case 'icon' :
				...
				$link = esc_url(add_query_arg(array('post' => $post->ID,
					'action' => 'edit'), 'post.php'));
				// If it post has a featured image.
				if(has_post_thumbnail()){
					// Display post featured image with edit link.
					echo '<a href="' . $link . '">';
					the_post_thumbnail(array(60,
						60));
					echo '</a>';
			...
			default :
				...
				if(isset($this->custom_populate_columns) && is_array($this->custom_populate_columns)){
					...
					if(isset($this->custom_populate_columns[$column]) && is_callable($this->custom_populate_columns[$column])){
						// Run the function.
						call_user_func_array($this->custom_populate_columns[$column], array($column,
							$post));
	...
	function sortable($columns = array()){
		...
		$this->add_filter('manage_edit-' . $this->post_type_name . '_sortable_columns', array(&$this,
			'make_columns_sortable'));
		// Run action that sorts columns on request.
		$this->add_action('load-edit.php', array(&$this,
			'load_edit'));
	}
	...
	function load_edit(){
		...
		$this->add_filter('request', array(&$this,
			'sort_columns'));
		...
		foreach($this->sortable as $column => $values){
			...
			if(isset($vars['post_type']) && $this->post_type_name == $vars['post_type']){
				...
				if(isset($vars['orderby']) && $meta_key == $vars['orderby']){
					...
					$vars = array_merge($vars, array('meta_key' => $meta_key,
						'orderby' => $orderby));
				}
			}
		}
	 ...
	 * dashicons are used. For more information see @link https://developer.wordpress.org/resource/dashicon
	 *
	...
	function updated_messages($messages){
		$post = get_post();
		$singular = $this->singular;
		$messages[$this->post_type_name] = array(0 => '',
	...
	function bulk_updated_messages($bulk_messages, $bulk_counts){
		$singular = $this->singular;
		$plural = $this->plural;
		$bulk_messages[$this->post_type_name] = array('updated' => _n('%s ' . $singular . ' updated.', '%s ' . $plural . ' updated.', $bulk_counts['updated']),
